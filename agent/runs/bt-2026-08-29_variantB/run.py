#!/usr/bin/env python3
"""VARIANT B — wheel CSP->CC backtest, three pre-committed levers.

Same engine/data as bt-2026-08-29_wheel-csp-cc_1Day (the published baseline),
plus: (1) SMA200 quality screen on new CSPs, (2) trend-regime-scaled delta
(RISK_ON 0.40 / NEUTRAL 0.30 / RISK_OFF 0.20) + looser CC band in RISK_ON,
(3) 4% APR interest on uninvested cash (incl. CSP collateral). Parameters
PRE-COMMITTED before looking at OOS; walk-forward split in summary.json.
Exploration only — does NOT replace the published baseline run.
...
WIN_START = date.fromisoformat(os.environ.get("BT_WIN_START", "2024-03-01"))
WIN_END = date.fromisoformat(os.environ.get("BT_WIN_END", "2026-07-31"))
INITIAL_CASH = 100_000.0
UNIVERSE = ["INTC", "T", "F", "GM", "PFE", "KO", "NOK", "SOFI", "BABA", "MU", "WBD", "VALE"]
SECTORS = {"semis": ["INTC", "MU"], "telecom": ["T", "NOK"], "auto": ["F", "GM"],
           "pharma": ["PFE"], "staples": ["KO"], "fintech": ["SOFI"],
           "china-ecom": ["BABA"], "media": ["WBD"], "materials": ["VALE"]}
...
PER_NAME_PCT, TOTAL_PCT = 0.18, 0.72
DRAWDOWN_STOP = 0.03
SPY_BEAR, SPY_EXTREME = -2.0, -4.0

# --- variant B levers (pre-committed; see notes.md) ---
CASH_APR = 0.04               # interest on uninvested cash incl. CSP collateral
QUALITY_SMA = 200             # new CSPs only when the name closes above its SMA200
RISKON_TARGET, RISKON_BAND = 0.40, (0.30, 0.45)   # aggressive tier (WheelMetrics)
BB_STD_RISKON = 1.5           # CC band multiplier when SPY trend is RISK_ON
WF_SPLIT = "2025-06-30"       # walk-forward: IS before, OOS after (inclusive)
...
class Backtest:
    def __init__(self, stocks, opts: OptFetch, divs):
        ...
        self.interest = 0.0                     # variant B: cash yield accrual
        ...
        self.gates = {"vol_floor": 0, "drawdown": 0, "extreme": 0, "no_candidate": 0,
                      ...
                      "bear_entries_scaled": 0, "cc_floor_block": 0, "quality": 0}
    ...
    def run(self):
        ...
        for i, day in enumerate(self.days):
            ...
            daily_int = self.cash * CASH_APR / 252   # variant B lever 3
            self.cash += daily_int
            self.interest += daily_int
    ...
    def _decision(self, day, nxt):
        ...
        regime = self._trend_regime(day)              # variant B lever 2
        risk_off = regime == "RISK_OFF"
        budget_mult = 0.5 if (bear or risk_off) else 1.0
        if regime == "RISK_ON":
            band, target = RISKON_BAND, RISKON_TARGET
        elif risk_off or bear:
            band, target = BEAR_BAND, BEAR_TARGET
        else:
            band, target = CSP_BAND, CSP_TARGET
        cc_std = BB_STD_RISKON if regime == "RISK_ON" else BB_STD

        # (a) new CSPs for flat names
        for u in UNIVERSE:
            if len(self.exposed()) >= MAX_UNDERLYINGS:
                self.gates["slots"] += 1
                break
            ...
            if self.sector_count(u) >= MAX_PER_SECTOR:
                self.gates["sector"] += 1
                continue
            sma_q = self._sma_u(u, day, QUALITY_SMA)   # variant B lever 1
            if sma_q and self.spot(u, day) < sma_q:
                self.gates["quality"] += 1
                continue
            closes = self.closes_upto(u, day)
            vp = vol_dollar_percentile(closes)
            if vp is not None and vp < VOL_FLOOR:
                self.gates["vol_floor"] += 1
                continue
            strike_max = PER_NAME_PCT * eq / 100.0
            best = self.best_candidate(u, day, "P", band, target, strike_max=strike_max)
            if best is None:
                self.gates["no_candidate"] += 1
                continue
            budget = min(PER_NAME_PCT * eq,
            ...
            if qty < 1:
                self.gates["budget"] += 1
                continue
            if self.cash < best["strike"] * 100 * qty:
                self.gates["budget"] += 1
                continue
            if bear:
        ...
        for u, sh in list(self.shares.items()):
            if u in {c["u"] for c in self.calls.values()} or sh["qty"] < 100:
                continue
            closes = self.closes_upto(u, day)
            ub = upper_bollinger(closes, cc_std)
    ...
    def _sma_u(self, u, day, n):
        hist = self.closes_upto(u, day)
        if len(hist) < n:
            return None
        return sum(hist[-n:]) / n

    def _trend_regime(self, day):
        """Daily-close trend regime, no lookahead. RISK_OFF: SPY < SMA200.
        RISK_ON: SPY > SMA50 and SMA50 rising vs 20 sessions ago. Else NEUTRAL."""
        hist = sorted(t for t in self.spy_close if t <= day)
        if len(hist) < 200:
            return "NEUTRAL"
        closes = [self.spy_close[t] for t in hist]
        s200 = sum(closes[-200:]) / 200
        if closes[-1] < s200:
            return "RISK_OFF"
        s50 = sum(closes[-50:]) / 50
        s50_prev = sum(closes[-70:-20]) / 50 if len(closes) >= 70 else None
        if closes[-1] > s50 and s50_prev and s50 > s50_prev:
            return "RISK_ON"
        return "NEUTRAL"
...
def main():
    ...
    print("== simulate (single pass: opts_cache pre-verified by the baseline run) ==",
          flush=True)
    opts = OptFetch()
    bt = None
    for pass_no in (1,):
    ...
    # variant B: walk-forward segments (params pre-committed before OOS)
    split_i = next((i for i, p in enumerate(bt.equity_curve) if p["date"] > WF_SPLIT),
                   max(len(bt.equity_curve) - 1, 1))
    wf = {}
    for name, lo, hi in (("IS", 0, split_i), ("OOS", split_i - 1, len(eq))):
        wf[name] = {"strategy": metrics(eq[lo:hi]),
                    "benchmark_spy": metrics(spy_eq[lo:hi])}
    ...
    summary = {
        ...
        "variant_levers": {"cash_apr": CASH_APR, "quality_sma": QUALITY_SMA,
                           "riskon_delta": RISKON_TARGET, "cc_std_riskon": BB_STD_RISKON},
        "walk_forward": {"split_after": WF_SPLIT, "segments": wf},
        "cash_interest_accrued": round(bt.interest, 2),
    }

    # ---- artifacts ----
    (RUN / "summary.json").write_text(json.dumps(summary, indent=2))
    with open(RUN / "trades.csv", "w", newline="") as f:
    ...
    with open(RUN / "round_trips.csv", "w", newline="") as f:
    ...
    (RUN / "data_fingerprint.json").write_text(json.dumps(fp, indent=2))
    (RUN / "warnings.json").write_text(json.dumps({
    ...
    (RUN / "fee_source.json").write_text(json.dumps({
    ...
    report = f"""# Variant B report — quality screen + regime delta + cash yield
...
Variant levers: SMA{QUALITY_SMA} quality screen · regime delta (RISK_ON {RISKON_TARGET} / NEUTRAL {CSP_TARGET} / RISK_OFF {BEAR_TARGET}) · CC band {BB_STD_RISKON}σ in RISK_ON · cash {CASH_APR:.0%} APR (accrued ${bt.interest:,.0f}).

## Walk-forward — parameters pre-committed before OOS; split after {WF_SPLIT}
| Segment | Strategy | SPY buy-hold |
|---|---|---|
| IS  | {wf['IS']['strategy']['total_return']}% · DD {wf['IS']['strategy']['max_drawdown']}% · Sharpe {wf['IS']['strategy']['sharpe']} | {wf['IS']['benchmark_spy']['total_return']}% · DD {wf['IS']['benchmark_spy']['max_drawdown']}% · Sharpe {wf['IS']['benchmark_spy']['sharpe']} |
| OOS | {wf['OOS']['strategy']['total_return']}% · DD {wf['OOS']['strategy']['max_drawdown']}% · Sharpe {wf['OOS']['strategy']['sharpe']} | {wf['OOS']['benchmark_spy']['total_return']}% · DD {wf['OOS']['benchmark_spy']['max_drawdown']}% · Sharpe {wf['OOS']['benchmark_spy']['sharpe']} |
...
"""
    ...
    # ---- dashboard export: DISABLED for variant B ----
    # The published agent/dashboard/backtest.json belongs to the baseline
    # run; this exploration keeps its artifacts inside its own folder.
