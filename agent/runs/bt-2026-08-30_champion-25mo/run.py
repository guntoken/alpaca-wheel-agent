#!/usr/bin/env python3
"""VARIANT C — Weekly Champion Wheel: selection-first, 1 name at a time.

WheelMetrics-style "stock selection BEFORE the options scan": each Monday the
bot scores an enlarged universe (quality gate close>SMA200, then momentum-63d
rank + vol-dollar-percentile rank, affordability), and sells a CSP ONLY on the
champion (top-3 fallback if no viable contract). Concentration: per-name
budget 18% -> 40%. Window: 1 year (2025-08-01 -> 2026-07-31). Everything else
= baseline engine (delta 0.30, DTE 7-35, TP 50%, roll-credit at 0.60, caps).
Parameters PRE-COMMITTED before seeing results. Exploration, not the headline.
...
SECTORS = {"semis": ["INTC", "MU", "AMD", "NVDA"], "telecom": ["T", "NOK"],
           ...
           "consumer": ["NKE"]}
CHAMPION_TOPK = 3          # coba juara, lalu runner-up, lalu ke-3
...
class Backtest:
    ...
    def _decision(self, day, nxt):
        ...
        # (a) WEEKLY CHAMPION — selection BEFORE the options scan (variant C):
        # score the universe, then sell ONE CSP on the week's best name only.
        if len(self.exposed()) < MAX_UNDERLYINGS and self.cash > 0:
            ...
            for u in UNIVERSE:
                ...
                if vp is None or vp < VOL_FLOOR:
                    ...
                    continue
                if closes[-1] * 100 > PER_NAME_PCT * eq:   # >= 1 kontrak tak terjangkau
                    self.gates["afford"] += 1
                    continue
            ...
            if not scored:
                ...
                for rank, (u, _m, _v) in enumerate(ranked[:CHAMPION_TOPK], 1):
                    ...
                    print(f"    -> CSP #{rank} {u} {best['occ']} x{qty} "
                    ...
                    self.queue.append({"kind": "SELL_PUT", "occ": best["occ"], "u": u,
                                       ...
                                       "decided": day})
                    break                      # satu CSP per minggu — esensi varian C
...
def main():
    ...
    summary = {
        ...
        "variant": "C weekly-champion (selection-first, 1 name, 40% budget)",
    ...
    report = f"""# Variant C report — Weekly Champion Wheel (1 tahun, selection-first)
