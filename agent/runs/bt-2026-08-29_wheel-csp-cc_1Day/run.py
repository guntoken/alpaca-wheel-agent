#!/usr/bin/env python3
"""Wheel CSP->CC backtest on real Alpaca data — single-file, deterministic.

Mirrors agent/src/agent/wheel.py + config.py exactly (no re-tuning).
...
WIN_START, WIN_END = date(2024, 3, 1), date(2026, 7, 31)
...
def main():
    ...
    print("== simulate (self-healing: empty ladders refetched in fresh processes) ==",
          flush=True)
    opts = OptFetch()
    bt = None
    ...
    report = f"""# Backtest report — wheel CSP→CC, real Alpaca option data
...
"""
    ...
    curve = [{"date": t, "strategy": round(e, 0), "spy": round(s, 0)}
             for t, e, s in zip(spy_days, eq, spy_eq)]
    dash = {
        "generated_at": datetime.utcnow().isoformat() + "Z",
        "run_folder": "agent/runs/" + RUN.name,
        "window": summary["window"],
        "data": "Real Alpaca option-trade daily bars (premiums) + Alpaca CLI stock bars "
                "(sip, raw) + Alpaca corporate-actions feed. Horizon set by Alpaca's "
                "historical option-data coverage (from Jan 2024).",
        "methodology": [
            "Rules mirror the live engine 1:1 (wheel.py + config.py) — zero re-tuning",
            "CSP: delta 0.18-0.42 (target 0.30), DTE 7-35, premium >= 0.5% of collateral, "
            "vol-percentile >= 40th of own history",
            "TP at 50% of credit; roll only for net credit at delta 0.60; assignment "
            "-> covered calls delta 0.12-0.38 above max(cost, upper Bollinger)",
            "Caps: 5 underlyings, 18% per name, 72% total, 2 per sector, -3% daily "
            "drawdown stop, SPY regime anchor halves budget below -2%",
            "Fills: next-day open with 5% adverse slippage, $0.50/contract commission",
            "The AI layer is NOT replayed: it can only veto/halve, so this run bounds "
            "the deterministic core from above",
        ],
        "strategy": strat_metrics, "benchmark_spy": spy_metrics,
        "option_legs": summary["option_legs"],
        "premiums_collected_gross": round(premiums, 0),
        "monthly_premium": [{"month": k, "premium": round(v, 0)}
                            for k, v in sorted(bt.monthly.items())],
        "activity": counts, "gate_activity": bt.gates,
        "equity_curve": curve,
        "first_trade": first_trade, "last_trade": last_trade,
        "caveats": "Hypothetical simulation: next-open fills, volume liquidity proxy, "
                   "inverse-BS greeks, assignment only at expiry (no early assignment), "
                   "deterministic core only. Full list in the run folder's notes.md.",
        "disclosure": "Backtest is a hypothetical historical simulation; results do not "
                      "guarantee future results; research/educational only; not "
                      "investment advice. Alpaca disclosures: alpaca.markets/disclosures",
    }
    (DASH / "backtest.json").write_text(json.dumps(dash))
